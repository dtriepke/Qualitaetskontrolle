...
* `proc.cap.confi` = Konfidenzintervalle fuer `proc.cap` mit `bias = TRUE` und  `proc.cap.k` mit `bias = FALSE`
...
(*Anmerkung: während die Werte für $C_p$ noch im akzeptablen Bereich liegen, ist das für $C_{pk}$ nicht der Fall*)
...
# Aufgabe 3
Bestimmen der Operationscharacteristrik eines einfachen Prüfplans mit:
...
Die Funktion zur Berechnung der Operationscharacteristik:
```{r}
...
Die Funktion `L_p_func` aufgerufen mit dem angegeben Parametern der Aufgabe ergit dann die nachfolgende OC:
```{r}
...
Zu Beginn wird die Funktion `find_n_c` programmiert um den optimalen Plan zu finden. 
```{r}
